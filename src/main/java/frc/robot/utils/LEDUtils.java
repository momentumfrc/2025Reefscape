...
public class LEDUtils {
    ...
    public static LEDPattern clip(LEDPattern pattern, int startPos, int endPos) {
        return (reader, writer) -> {
            int adjustedEndPos = endPos < 0 ? reader.getLength() : endPos;
            pattern.applyTo(reader, (i, r, g, b) -> {
                if (i < startPos || i >= adjustedEndPos) {
                    return;
                }
    ...
    public static LEDPattern stack(LEDPattern base, LEDPattern... overlays) {
        return (reader, writer) -> {
            base.applyTo(reader, writer);
            for (LEDPattern pattern : overlays) {
